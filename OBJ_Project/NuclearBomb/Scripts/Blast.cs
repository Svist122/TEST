...
public class Blast : MonoBehaviour {
	...
	void OnTriggerEnter(Collider col) {		
		if (col.tag == "Destroy") { if (col.name == "Build")
			{
		...
		if (Random.value > .05f || Vector3.Distance (go.transform.position, transform.position) < 100) {
			...
			if (debris.Length > 0 && generateDebris) {
				...
				for (int i = 0; i < (int)(debrisCount * go.transform.localScale.magnitude); i++) {
					GameObject g = Instantiate (debris[Random.Range (0, debris.Length - 1)], go.transform.position + Random.insideUnitSphere * go.transform.localScale.x, go.transform.rotation) as GameObject;
					g.SetActive (true);
					g.hideFlags = HideFlags.HideInHierarchy;
					//g.AddComponent <Rigidbody> ().mass = .1f; 
					//g.GetComponent <Rigidbody> ().AddExplosionForce (80, transform.position, 500);
					g.AddComponent <AutoDestroyInXSeconds> ().t = Random.Range (1.2f, 1.7f);
				}
